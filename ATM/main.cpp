...
int main()
{
	...
	int arr[] = { 1000,2000,5000,10000,20000,50000 }; //массив, где в [0] хранятся купюры по 100 рублей, [1] - по 200 рублей и т.д.
	int cash, cashF, summa, result100, result200, result500, result1000, result2000, result5000;
	summa = 0; //инф. для понимания всей суммы
	for (int i = 0; i < 6; i++) {
		summa = summa + arr[i];
	}
	...
	if (cash >= 5000) { //проверка на операцию больше или равную 5000
		if (cash / 5000 <= arr[5] / 5000) { //если операция укладывается в лимит элемента массива, то производит дальнейший расчет
